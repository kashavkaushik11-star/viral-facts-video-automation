...
async function getGoogleTrends() {
  ...
  const items = [...xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)]
    .map(match => {
      ...
      return {
        ...
        traffic: traffic ? decodeXml(traffic) : '',
        pubDate: pubDate ? decodeXml(pubDate) : ''
      };
    })
  ...
  // This project is now a TREND EXPLAINER, not a generic facts channel.
  // Keep the actual trending topic regardless of language or category.
  // Only remove obvious live-score metadata that is poor for an explainer.
  const blocked = /\b(scorecard|standings|fixture|live score|odds)\b/i;
  const candidates = items.filter(item => !blocked.test(item.title)).slice(0, 20);

  return {
    source: TRENDS_URL,
    fetchedAt: new Date().toISOString(),
    candidates: candidates.length ? candidates : items.slice(0, 20)
  };
}
