...
function createSrt(text, duration, outPath) {
  ...
  pieces.forEach((piece, index) => {
    ...
    const start = cursor;
    const end = index === pieces.length - 1 ? duration : Math.min(duration, cursor + pieceDuration);
...
function buildReel(imagePath, aiVideoPath, audioPath, srtPath, finalPath) {
  ...
  const aiVertical = '/tmp/ai_vertical.mp4';
  const stillVideo = '/tmp/still_zoom.mp4';
  const visualVideo = '/tmp/visual_reel.mp4';

  console.log(`Voice duration: ${audioDuration.toFixed(2)}s`);
  console.log(`Building ${audioDuration.toFixed(2)}s vertical reel...`);

  runFfmpeg([
    '-i', aiVideoPath,
    '-t', aiDuration.toFixed(2),
    '-vf', 'scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,fps=30,format=yuv420p',
    '-an', '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '20', aiVertical
  ]);

  runFfmpeg([
    '-loop', '1', '-i', imagePath, '-t', stillDuration.toFixed(2),
    '-vf', "scale=1920:1920,crop=1080:1920,zoompan=z='min(zoom+0.0015,1.12)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s=1080x1920:fps=30,format=yuv420p",
    '-an', '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '20', stillVideo
  ]);

  runFfmpeg([
    '-i', aiVertical, '-i', stillVideo,
    '-filter_complex', '[0:v][1:v]concat=n=2:v=1:a=0[v]',
    '-map', '[v]', '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '20', '-pix_fmt', 'yuv420p', visualVideo
  ]);
...
(async () => {
  ...
  const aiVideoPath = path.join(outDir, 'ai_motion.mp4');
